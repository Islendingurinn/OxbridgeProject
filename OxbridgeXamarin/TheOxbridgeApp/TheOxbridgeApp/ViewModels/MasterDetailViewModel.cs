    ...
    public class MasterDetailViewModel : BaseViewModel
    {
        ...
        public async Task OnAppearing()
        {
            ...
            if (savedUser != null)
            {
                User user = new User();
                user.email = UserName;
                serverClient.Login(savedUser.email, savedUser.password);
                if (user != null)
                {
                    ...
                    //if there is a user present within this scope, we're adding the accountpage link here
                    MenuItems.Add(new MasterMenuItems()
                    {
                        Text = "Account",
                        ImagePath = "userIcon.png", //to be changed to current users profile pic
                        TargetViewModel = typeof(AccountPageViewModel)
                    });
